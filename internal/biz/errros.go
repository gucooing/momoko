...
var (
	...
	ErrEmailNot              = response.BadRequest(400, "邮箱未启用")
	ErrEmailNotFound         = response.BadRequest(400, "邮件模板未配置")
	ErrEmailInvalid          = response.BadRequest(400, "邮箱格式不正确")
	ErrEmailRegistered       = response.BadRequest(400, "邮箱已注册")
	ErrEmailCodeInvalid      = response.BadRequest(400, "邮箱验证码错误或已过期")
	ErrEmailTemplateType     = response.BadRequest(400, "邮件模板类型不正确")
	ErrEmailTemplateSubject  = response.BadRequest(400, "邮件主题不能为空")
	ErrEmailTemplateContent  = response.BadRequest(400, "邮件模板内容不能为空")
