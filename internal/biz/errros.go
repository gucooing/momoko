...
var (
	...
	ErrNotLoginType               = response.BadRequest(500, "不可用的登录方式")
	ErrNotDeviceID                = response.BadRequest(500, "device_id 不能为空")
	...
	ErrLotteryDisabled            = response.BadRequest(500, "抽奖活动未开启")
	ErrLotteryClosed              = response.BadRequest(500, "报名已截止")
	ErrLotteryDrawNotDue          = response.BadRequest(500, "未到开奖时间")
	ErrLotteryNotEligible         = response.BadRequest(500, "扣费未达到报名门槛")
	ErrLotteryTokenInvalid        = response.BadRequest(500, "用户令牌无效")
	ErrLotteryRoundNotFound       = response.BadRequest(500, "轮次不存在")
	ErrLotteryOutsideWindow       = response.BadRequest(500, "当前不在报名窗口，无法结算新轮次")
