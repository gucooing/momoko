...
var (
	...
	ErrImageGenTokenInvalid       = response.BadRequest(401, "sub2api 登录已过期，请从控制台重新进入")
	ErrImageGenDisabled           = response.BadRequest(403, "生图功能未启用")
	ErrImageGenSrcHostForbidden   = response.BadRequest(403, "该 sub2api 站点未获授权调用生图")
	ErrImageGenApiKeyInvalid      = response.BadRequest(400, "apikey 无效或不存在")
	ErrImageGenGenerationNotFound = response.BadRequest(404, "生图任务不存在")
	ErrImageGenImageNotFound      = response.BadRequest(404, "图片不存在")
	ErrFileSourceNotFound         = response.BadRequest(404, "文件来源不存在")
	ErrFileSourceDisabled         = response.BadRequest(403, "文件来源已停用")
	ErrFileSourceUnsupported      = response.BadRequest(400, "当前文件来源不支持该操作")
	ErrFileSourceType             = response.BadRequest(400, "文件来源类型不合法")
	ErrShareSignInvalid           = response.BadRequest(401, "分享会话已失效，请重新验证")
)
