...
func (_u *InstanceUpdate) check() error {
	...
	if _u.mutation.UserCleared() && len(_u.mutation.UserIDs()) > 0 {
		return errors.New(`gen: clearing a required unique edge "Instance.user"`)
	}
	if _u.mutation.TypeCleared() && len(_u.mutation.TypeIDs()) > 0 {
		return errors.New(`gen: clearing a required unique edge "Instance.type"`)
	}
	return nil
}
...
func (_u *InstanceUpdateOne) sqlSave(ctx context.Context) (_node *Instance, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`gen: missing "Instance.id" for update`)}
	}
