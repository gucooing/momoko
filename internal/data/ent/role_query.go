...
func (_q *RoleQuery) loadMenus(ctx context.Context, query *MenuQuery, nodes []*Role, init func(*Role), assign func(*Role, *Menu)) error {
	...
	for _, n := range neighbors {
		fk := n.role_menus
		if fk == nil {
			return fmt.Errorf(`foreign-key "role_menus" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "role_menus" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
