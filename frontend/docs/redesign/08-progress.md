...
| 2026-07-21 | **i18n 高优先第二批**：`docker.common.status`/`sub2api.common.status`；工作台问候/实时监控/查看全部/空态/运行中；ActionMenu 默认 `common.operation`；tunnel/port-forward 移动「连接」→`common.connectionsUnit`。OIDC 整页硬编码仍待。**待提交。** | 提交；OIDC i18n 或 a11y 余量 |
