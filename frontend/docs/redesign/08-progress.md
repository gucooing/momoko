...
| 2026-07-21 | **OIDC 管理页全量 i18n**：`system.oidc.*` 三语；`index`/`clientForm`/`configDialog` 去硬编码中文（列表/筛选/表头/行菜单/删除·刷新确认/reveal/配置弹窗/创建编辑校验 toast）。浏览器：列表+配置弹窗暗色文案 OK。**待提交。** | 提交；a11y 余量 / 可选 --el→--app |
