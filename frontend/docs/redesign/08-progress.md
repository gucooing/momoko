...
> **一句话现状（换设备从这里接）**：**前端从零重写 + EP/VXE 卸载已收口**；终端 M-2 + 实例文件已提交（`18834f1`）。**本会话 a11y/i18n 抽查落地**：补 `common.status`/`system.common.status` 等缺失键；全局 `--app-focus-ring` + `:focus-visible`；粗指针命中盒；PageHeader 移动按钮 36px；settings 预览 iframe 去死白；placeholder 对比度略加深。**下一步**：继续 a11y 扫（PermissionTree/DataTable 复选框命中、imagine prompt focus）；可选 `--el-*`→`--app-*`；提交本批 a11y/i18n。⚠️ 改 Go 后需用户在 GoLand 重跑后端。
...
| 2026-07-21 | **终端 M-2 + 实例文件收口**：①`TerminalShell` 移动软键盘（≤768：Ctrl/Alt 修饰 + Esc/Tab/C/D/Z/L/方向/Home/End/PgUp/PgDn，序列注入 `@data`）+ 顶栏 A± 字号 + 底栏 `· Npx`；`useTerminalX` 字号 localStorage 共享热更；浅色 `--term-*` 验过。②实例文件页 `useFullBleed`；**修** 列表/控制台把 `instancePath`（如 `./servers/mc`）当 `workdir` 导致实例 API 500——实例文件根即实例目录，不再透传。浏览器：files 桌面1440/移动390 列表正常；console 390 暗/浅软键盘+字号 13→14、桌面无软键盘。vue-tsc/eslint 绿。**已提交 `18834f1`。** | 暗色/a11y/i18n 抽查 |
| 2026-07-21 | **a11y/i18n 抽查首批落地**：①缺键 `common.status` + `system.common.{status,loading,serialNumber}` 三语；②全局 focus-visible/触控命中/PageHeader 移动 36px；settings iframe；placeholder 对比度。**已提交 `11bf254`。** | 继续 i18n 高优先 |
| 2026-07-21 | **i18n 高优先第二批**：`docker.common.status`/`sub2api.common.status`；工作台问候/实时监控/查看全部/空态/运行中；ActionMenu 默认 `common.operation`；tunnel/port-forward 移动「连接」→`common.connectionsUnit`。**已提交 `6414649`。** | OIDC i18n |
| 2026-07-21 | **OIDC 管理页全量 i18n**：`system.oidc.*` 三语；`index`/`clientForm`/`configDialog` 去硬编码中文。浏览器列表+配置弹窗 OK。**已提交 `6b5269e`。** | a11y 触控/焦点余量 |
| 2026-07-21 | **a11y 触控/焦点第二批**：PermissionTree caret/checkbox 伪元素扩命中 + focus-visible；DataTable 树 caret/原生 checkbox 焦点与 coarse 放大；UserPicker chip × 扩命中；imagine `composer-bar:focus-within` 环 + mode-seg 焦点 + 移动 mode 按钮 36px；全局 focus 选择器含 role=checkbox/tab。vue-tsc 绿。**待提交。** | 提交；可选 --el→--app / 更多 i18n 扫 |
