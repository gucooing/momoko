...
> **一句话现状（换设备从这里接）**：Phase 0–2 + Phase 3 基本完成。公开门户全部去 EP：`home/stats`(`ea7c1d4`)、`imagine`+`lottery`(`9c963f8`)、`activity`(`24fa5e7`)；**`public/share`（el-icon 去净、留 fm-* 文件主题）+ `oidc/authorize`（全量重写）去 EP 完成并全验，已提交 `7ae41db`**。监控假数据→Phase 4。**下一步**：Phase 4（卸 EP/VXE + 清 demo/extended + 暗色/可访问性/i18n 全量核对）。
