...
| 2026-07-21 | **a11y/i18n 抽查首批落地**：①缺键 `common.status` + `system.common.{status,loading,serialNumber}` 三语（列表筛选项原显示 raw key）；②`design-tokens` 全局 `--app-focus-ring` + button/role=button/a/自建控件 `:focus-visible`；粗指针 `.app-icon-btn`/ActionMenu≥40；PageHeader 移动动作 36px；settings `preview__iframe` 去 `#fff`；placeholder 浅 `#7a8494` / 暗 `#7a8496`。浏览器：user/instance 暗色「状态」已译。**未提交。** | 提交本批；继续 a11y 扫 |
