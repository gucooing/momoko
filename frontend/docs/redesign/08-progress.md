...
| Phase 4 | 清理待决 + 暗色/可访问性/i18n 全量核对 + **EP/VXE 彻底卸载** | 🟡 **Slice1**(删 demo/extended/假分析监控/旧布局壳 37 文件 `6398d24` + 后端菜单裁剪 `1dbebed`)✅ · **Slice2**(BaseDialog→令牌模态，Dialog.* 24 文件 + 4 直接消费者全量透传去 EP `f0376a1`)✅ · 文件模块收尾(FilePicker/FilePager `c6a5c29`)✅ · 余 **Slice3**(ElMessage→useFeedback 单例，~44 文件)/**Slice4**(剩余共享组件 16 文件 `<el-`)/**Slice5**(卸 EP/VXE 包 + grep 清零)；`<el-` 组件 47→16 文件 |
...
> **一句话现状（换设备从这里接）**：Phase 0–3 完成。**Phase 4 进行中（EP/VXE 卸载）**：Slice1 删 37 文件死代码/demo/旧布局壳 + 后端菜单裁剪(`6398d24`/`1dbebed`)；Slice2 BaseDialog→令牌模态、Dialog.* 全量透传去 EP(`f0376a1`)；文件模块收尾(`c6a5c29`)；公开门户 share/oidc(`7ae41db`)。`<el-` 组件 47→16 文件。**下一步 Slice3**：`ElMessage`/`ElMessageBox`→`useFeedback`（~44 文件；需先建 toast 单例桥供 `utils/request` 拦截器/stores 等非组件上下文用）。再 Slice4（剩余 16 个 `<el-` 共享组件：AppShell el-drawer / App.vue el-config-provider / ThemeConfig / SelectAvatarDialog / IconSelectorDialog / BaseCard / BaseTag / TextEllipsis / TablePagination / IconButton / LoadingButton / BadgeTabsMenu / AdaptiveConfirm / UpdateReleaseContent / RunningInstanceCard / DockerTaskDialogs）→ Slice5 卸包 + 全仓 grep 清零。⚠️ 后端菜单裁剪需用户重跑 Go 生效。
