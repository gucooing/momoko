  ...
  <main class="s2a-stats" :class="{ 'is-dark': isDark }">
    ...
    <section class="wrap">
      ...
      <div v-loading="store.statsLoading" class="content">
        <div class="metrics">
          <div v-for="card in store.statsMetricCards" :key="card.label" class="metric" :class="`tone-${card.tone}`">
        ...
        <article class="panel">
          <div class="panel-head"><h3>用量趋势</h3><span>{{ stats?.rangeLabel }}</span></div>
          <VChart class="chart" :option="store.statsTrendOption" :update-options="chartUpdate" autoresize />
        </article>

        <article class="panel">
          <div class="panel-head"><h3>模型明细</h3><span>{{ models.length }} 个模型</span></div>
          <el-table :data="models" :max-height="288" stripe>
            <el-table-column type="index" label="#" width="56" />
            <el-table-column prop="name" label="模型" min-width="180" show-overflow-tooltip />
            <el-table-column label="请求数" width="120" sortable :sort-method="(a:any,b:any)=>a.requestCount-b.requestCount">
            ...
            <el-table-column label="生成速度" width="130">
              <template #default="{ row }">{{ store.formatThroughput(row.avgTps) }} token/s</template>
            </el-table-column>
        ...
        <article v-if="endpoints.length" class="panel">
          ...
          <el-table :data="endpoints" :max-height="288" stripe>
            <el-table-column type="index" label="#" width="56" />
...
.metric {
  ...
  &.tone-green { border-top-color: var(--el-color-success); }
  &.tone-amber { border-top-color: var(--el-color-warning); }
  &.tone-red { border-top-color: var(--el-color-danger); }
}

.m-label { color: var(--el-text-color-secondary); font-size: 12.5px; }
.m-value { color: var(--el-text-color-primary); font-size: 20px; font-weight: 800; font-variant-numeric: tabular-nums; }
.m-detail { color: var(--el-text-color-placeholder); font-size: 12px; }
...
.panel-head {
  ...
  span { color: var(--el-text-color-placeholder); font-size: 12.5px; }
}
