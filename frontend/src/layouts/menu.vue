<template>
  <el-scrollbar>
    ...
    >
      ...
      <MenuItem v-for="item in menuStore.menuList" :key="item.id" :item="item" />
    </el-menu>
  </el-scrollbar>
</template>
...
const isTopMenu = computed(() => themeStore.layout === 'topMode')

// 菜单模式
const menuMode = computed(() => {
  if (menuStore.isMobile) return 'vertical'
  return isTopMenu.value ? 'horizontal' : 'vertical'
})

// 菜单颜色配置
const menuBackgroundColor = computed(() => {
  if (isTopMenu.value) return 'var(--el-bg-color)'
  return isDarkTheme.value
    ? 'color-mix(in srgb, var(--el-color-primary) 16%, #141414)'
    : 'color-mix(in srgb, var(--el-color-primary) 8%, #ffffff)'
})

const menuTextColor = computed(() => {
  if (isTopMenu.value) return 'var(--el-text-color-primary)'
  return isDarkTheme.value ? '#e5e7eb' : '#303133'
})

const menuActiveTextColor = computed(() => themeStore.primaryColor)

const logoTitleColor = computed(() => {
  if (isTopMenu.value) return ''
  return menuTextColor.value
})
