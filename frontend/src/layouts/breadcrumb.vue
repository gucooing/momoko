...
const findHomeMenu = (menus: MenuInfo[]): MenuInfo | null => {
  for (const item of menus) {
    if (item.type === MenuType.MenuType_Menu && (item.path === '/' || item.path === '/dashboard/home')) {
      return item
    }
    if (item.children?.length) {
...
const buildMenuMap = (
  menus: MenuInfo[],
  map: Map<string, MenuInfo> = new Map(),
): Map<string, MenuInfo> => {
  menus.forEach((item) => {
...
const buildBreadcrumbPath = (
  ...
  let node: MenuInfo | null = current

  while (node) {
    if (node.type === MenuType.MenuType_Directory || node.type === MenuType.MenuType_Menu) {
      path.unshift({
        id: node.id,
        title: node.title,
        path: node.path || '#',
