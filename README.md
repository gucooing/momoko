...
- Sub2API 配置：公开首页、同步开关、服务地址、管理员 API Key、控制台地址、标题、副标题、介绍、同步间隔、历史天数和分页大小。
- 连接测试、使用记录同步、用量快照、后台统计、公开统计、最近请求。
- 公告和时间线管理。
- 公开页面：Sub2API 首页、Sub2API 统计页。
- 嵌入式生图：API Key 列表、模型列表、文生图、图生图、异步生成任务、历史任务、图片详情和任务删除。
- 生图接口使用 `X-Sub2API-Token` 鉴权，并支持来源站点白名单。
