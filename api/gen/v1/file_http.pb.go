...
const OperationFileManagerGetFileSystemTree = "/v1.FileManager/GetFileSystemTree"
...
type FileManagerHTTPServer interface {
	...
	// GetFileSystemTree 获取目录树(系统级，懒加载，返回指定目录的直接子项，供编辑器文件树使用)
...
func RegisterFileManagerHTTPServer(s *http.Server, srv FileManagerHTTPServer) {
	...
	r.GET("/api/v1/file/system/tree", _FileManager_GetFileSystemTree0_HTTP_Handler(srv))
...
func _FileManager_GetFileSystemTree0_HTTP_Handler(srv FileManagerHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in GetFileSystemTreeRequest
		if err := ctx.BindQuery(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, OperationFileManagerGetFileSystemTree)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.GetFileSystemTree(ctx, req.(*GetFileSystemTreeRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
		}
		reply := out.(*GetFileSystemTreeResponse)
		return ctx.Result(200, reply)
	}
}
...
type FileManagerHTTPClient interface {
	...
	// GetFileSystemTree 获取目录树(系统级，懒加载，返回指定目录的直接子项，供编辑器文件树使用)
	GetFileSystemTree(ctx context.Context, req *GetFileSystemTreeRequest, opts ...http.CallOption) (rsp *GetFileSystemTreeResponse, err error)
	// GetFileTask 获取文件任务状态
...
// GetFileSystemTree 获取目录树(系统级，懒加载，返回指定目录的直接子项，供编辑器文件树使用)
func (c *FileManagerHTTPClientImpl) GetFileSystemTree(ctx context.Context, in *GetFileSystemTreeRequest, opts ...http.CallOption) (*GetFileSystemTreeResponse, error) {
	var out GetFileSystemTreeResponse
	pattern := "/api/v1/file/system/tree"
	path := binding.EncodeURL(pattern, in, true)
	opts = append(opts, http.Operation(OperationFileManagerGetFileSystemTree))
	opts = append(opts, http.PathTemplate(pattern))
	err := c.cc.Invoke(ctx, "GET", path, nil, &out, opts...)
	if err != nil {
		return nil, err
